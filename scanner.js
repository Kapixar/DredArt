...
onmessage = (event) => {
    ...
    xSizesLeg = event.data.xSizesLeg;
    ySizesLeg = event.data.ySizesLeg;
    const w = xSizesLeg[0] + xSizesLeg[1];
    const h = ySizesLeg[0] + ySizesLeg[1];
    for (let i = 0; i < h; i++) {
        for (let o = 0; o < w; o++) {
            const j = pxIndex(o, i, w);
    ...
    postMessage({ imgColors, borderColors });
};
