...
function render(img) { 
    ...
    const   canScale= document.querySelector('#shipShape canvas'),
            ctxScale=canScale.getContext('2d', {willReadFrequently: true});
    ...
    canScale.width = img.width;
    canScale.height = img.height;
    ctxScale.drawImage(img, 0, 0);
    const sD = ctxScale.getImageData(0,0,img.width,img.height).data;
    ...
    function renderScale() {
        ...
        if(img.width<78) canScale.width=img.width;
        else canScale.width=78;
        if(img.height<78) canScale.height=img.height;
        else canScale.height=78;
        ...
        const ctxMove = canMove.getContext("2d");
        ctxMove.imageSmoothingEnabled = false;
        ...
        let minzoom, maxzoom;
        let maxWidth=canScale.width, maxHeight=canScale.height, defaultWidth, defaultHeight;
        ...
        const widthInp = document.querySelector("#sizebox input:first-of-type");        //size inputs
        widthInp.setAttribute("placeholder",defaultWidth.toString());
        const heightInp = document.querySelector("#sizebox input:nth-of-type(2)");
        heightInp.setAttribute("placeholder",defaultHeight.toString());


        const ratioCheck = document.querySelector("#sizebox label:last-of-type input");
        ratioCheck.onchange = function() {
            if(this.checked){
                widthInp.value='';
                heightInp.value='';
                widthInp.setAttribute("placeholder",defaultWidth);
                heightInp.setAttribute("placeholder",defaultHeight);
            } else {
                widthInp.setAttribute("placeholder",maxWidth.toString());
                heightInp.setAttribute("placeholder",maxHeight.toString());
            }
        }
        ...
        widthInp.oldValue='';
        heightInp.oldValue='';
        new Array(widthInp,heightInp).forEach(function(tbox) {                       //input handler
            tbox.addEventListener("input", function() {
                if(/^(?!0)\d{0,2}$/.test(this.value) && !(ratioCheck.checked && !((this==widthInp&&this.value<=defaultWidth)||(this==heightInp&&this.value<=defaultHeight))) && ((this==widthInp&&this.value<=maxWidth)||(this==heightInp&&this.value<=maxHeight))) {
                    ...
                    if(ratioCheck.checked) {
                        if(this.value!=''){                                  //aspect ratio
                            if(this==widthInp) {
                                ...
                                heightInp.value = h<maxHeight? Math.round(h) : '';
                                heightInp.oldValue = heightInp.value;
                                heightInp.classList.add('flash');
                                setTimeout(()=>heightInp.classList.remove('flash'), 500);
                            }else{
                                let w=img.width*this.value/img.height;
                                widthInp.value = w<maxWidth? Math.round(w) : '';
                                widthInp.oldValue = widthInp.value;
                                widthInp.classList.add('flash');
                                setTimeout(()=>widthInp.classList.remove('flash'), 500);
                            }
                        }else{
                            if(this==widthInp) heightInp.value = '';
                            else widthInp.value = ''; 
                        }
                    }
                    if(this.value.length>=2 && this==widthInp) heightInp.focus();
                    else if(this.value.length==0 && this==heightInp) widthInp.focus();
                } else {
        ...
        sizeApply.onclick = function() {
            if(widthInp.value!="")
                 canScale.width = parseInt(widthInp.value);
            else canScale.width=parseInt(widthInp.getAttribute("placeholder"));
            if(heightInp.value!="")
                 canScale.height = parseInt(heightInp.value);
            else canScale.height=parseInt(heightInp.getAttribute("placeholder"));

            if(canScale.height>=canScale.width){
                shipShape.height=400;
                shipShape.style.height="400px";
                shipShape.width=Math.round(canScale.width*400/canScale.height);
                shipShape.style.width=`${shipShape.width}px`;
            }else{
                shipShape.width=400;
                shipShape.style.width="400px";
                shipShape.height=Math.round(canScale.height*400/canScale.width);
                shipShape.style.height=`${shipShape.height}px`;
            }

            if(canScale.width/img.width>canScale.height/img.height) {
                minzoom=shipShape.width/img.width;
                maxzoom = shipShape.width/canScale.width
            }
            else {
            ...
            if(transparency){
                ...
                ctxScale.fillStyle = `rgb(${c[0]},${c[1]},${c[2]}")`;
            }
            renderShipBg();
            zoomCanvas(minzoom);
            scaleAndDither();
        ...
        const hideCheck = document.querySelector("#settings > label input")         //hide canvas
        hideCheck.onchange = function() {
            if(!this.checked) scaleAndDither()
            else shipShape.classList.add("hidden")
        }

        const shipBgCheck = document.querySelector("#settings > label:nth-of-type(2) input")     //ship background tiles
        shipBg.classList.add("hidden");
        shipBgCheck.onchange = function() {
            shipBg.classList.toggle("hidden");
        }

        const ditherCheck = document.querySelector("#settings > label:nth-of-type(3) input")    //dither switch
        ditherCheck.onchange = function() {dither();}


        const alghCheck = document.querySelector("#settings > label:nth-of-type(4) input")      //cie76 or cie94 switch
        alghCheck.onchange = function() {dither();}
        
    


        const hueValue = document.querySelector("#settings > label:nth-of-type(5) :last-child")        //hue rotate
        const hueRange = document.querySelector("#settings > label:nth-of-type(5) input")
        ...
        const saturationValue = document.querySelector("#settings > label:nth-of-type(6) :last-child") //saturation
        const saturationRange = document.querySelector("#settings > label:nth-of-type(6) input")
        ...
        const brightnessValue = document.querySelector("#settings > label:nth-of-type(7) :last-child") //brightness
        const brightnessRange = document.querySelector("#settings > label:nth-of-type(7) input") 
        ...
        const contrastValue = document.querySelector("#settings > label:nth-of-type(8) :last-child") //contrast
        const contrastRange = document.querySelector("#settings > label:nth-of-type(8) input")
        ...
        trackTransforms(ctxMove);
        redraw();
		function redraw(){
			var p1 = ctxMove.transformedPoint(0,0);
			var p2 = ctxMove.transformedPoint(canMove.width,canMove.height);
			ctxMove.clearRect(p1.x,p1.y,p2.x-p1.x,p2.y-p1.y);
			ctxMove.drawImage(img,0,0);
		}
		
		var lastX=canMove.width/2, lastY=canMove.height/2, dragStart, dragged;
		canMove.addEventListener('mousedown',(evt)=>{
			lastX = evt.offsetX;
			lastY = evt.offsetY;
			dragStart = ctxMove.transformedPoint(lastX,lastY);
            dragged=false;
		});
		canMove.addEventListener('mousemove',(evt)=>{
			lastX = evt.offsetX;
			lastY = evt.offsetY;
            dragged=true;
			if (dragStart){
                shipShape.classList.add("hidden");
				let pt = ctxMove.transformedPoint(lastX,lastY);
				ctxMove.translate(pt.x-dragStart.x,pt.y-dragStart.y);
                checkBoundaries();
                redraw();
			}
		});
		canMove.addEventListener('mouseup',()=>{
			dragStart = null;
            if(dragged) scaleAndDither();
		});
        canMove.addEventListener('mouseleave',()=>{
            if(dragStart && dragged) scaleAndDither();
			dragStart = null;
		});

		var scaleFactor = 1.06, scale=1;
		canMove.addEventListener('wheel', evt => wheelZoom(evt));
        ...
        function wheelZoom(evt){
            ...
            var pt = ctxMove.transformedPoint(lastX,lastY);
            ctxMove.translate(pt.x,pt.y);
            ctxMove.scale(factor,factor);
            scale *= factor;
            ctxMove.translate(-pt.x,-pt.y);
        ...
        function checkBoundaries() {
            ...
            var tr=ctxMove.getTransform(), top=250-shipShape.height/2, side=250-shipShape.width/2;
            if(tr['e']>side)
                ctxMove.translate((side-tr['e'])/scale, null);
            if(tr['f']>top)
                ctxMove.translate(null, (top-tr['f'])/scale);
                
            tr=ctxMove.getTransform();
            if(tr['e']+img.width*scale<500-side) 
                ctxMove.translate((500-side-tr['e']-img.width*scale)/scale, null);
            if(tr['f']+img.height*scale<500-top) 
                ctxMove.translate(null, (500-top-tr['f']-img.height*scale)/scale);
        }
        ...
        function rangeZoom() {
            ...
            if(scale*factor<minzoom)
                ...
                factor=maxzoom/scale;               
            scale *= factor;
            var pt = ctxMove.transformedPoint(250,250);
        ...
        function zoomCanvas(newScale) {
            if(newScale>=minzoom){
                var pt = ctxMove.transformedPoint(250,250);
                ctxMove.translate(pt.x,pt.y);
                ctxMove.scale(newScale/scale,newScale/scale);
                scale=newScale;
                ctxMove.translate(-pt.x,-pt.y);
                checkBoundaries();
                redraw();
        ...
        function filterImg() {
            ctxMove.filter = `hue-rotate(${hueRange.value}deg) saturate(${saturationRange.value}%) contrast(${contrastRange.value}%) brightness(${brightnessRange.value}%)`;
            redraw();
            scaleAndDither();
        }
        ...
        function renderShipBg() {
            ...
            let factor = 400 / Math.max(canScale.width, canScale.height);
            if(factor>64) factor=64;
            shipBg.width=factor*canScale.width;
            shipBg.height=factor*canScale.height;
        ...
        function scaleAndDither(){
            ...
            canTemp.height=shipShape.height;                                           //here real value
            ctxTemp.drawImage(canMove, 250-shipShape.width/2, 250-shipShape.height/2, shipShape.width, shipShape.height, 0,0, canTemp.width, canTemp.height);
            var b = Blitz.create();
            b({
                source: canTemp,
                width: canScale.width,
                height: canScale.height,
                output: 'canvas'
            }).then(output => {
                if(transparency) {
                    ctxSave.clearRect(0,0,canSave.width,canSave.height);
                    ctxScale.fillRect(0,0,canScale.width,canScale.height);
                }
        ...
        function dither(){
            ctxScale.drawImage(canSave,0,0);
            ...
            if(!ditherCheck.checked){                                                       //no dithering
                const scaleD = ctxScale.getImageData(0,0,canScale.width,canScale.height);
                let sD=scaleD.data;
                ...
                ctxScale.putImageData(scaleD,0,0);
            }else{                                                                          //floyd-steinberg
                const scaleD = ctxScale.getImageData(0,0,canScale.width,canScale.height);
                let sD=scaleD.data;
                for(var y=0; y<canScale.height; y++){
                    for(var x=0; x<canScale.width; x++){
                        let i=pxIndex(x,y);
    ...
    function renderDownload(){
        ...
        thumbnail.src = canScale.toDataURL();
    ...
    function renderMap(){
        return new Promise(resolve =>{
            ...
            let scale=20;
            canMap.width=canScale.width*scale;
            canMap.height=canScale.height*scale+25;
            ...
            ctxMap.drawImage(canScale,0,0);
            ctxMap.scale(1/scale,1/scale);
            let data = ctxScale.getImageData(0, 0, canScale.width, canScale.height).data;
            for(let i=0; i<canScale.height; i++){
                for(let o=0; o<canScale.width; o++){
                    let id = (o+i*canScale.width)*4;
