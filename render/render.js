...
document.onpaste = function (e) { // paste handler
    ...
    if (blob == undefined) { return error.textContent = 'Error. Pasted data is not an image.'; }
...
document.querySelector('#pngInp').onchange = function () { // file input handler
    if (this.files[0] == null) { return error.textContent = 'Error. Chose file again.'; }

    if (this.files.length > 1) { return error.textContent = 'Error. You can select only one file.'; }

    const file = this.files[0];
    this.value = null;

    if (file.length === null) { return error.textContent = 'Error. Propably file is empty'; }
...
drop.ondrop = function (ev) { // drop handlers
    ...
    if (ev.dataTransfer.files) {
        if (ev.dataTransfer.files.length == 0 || !ev.dataTransfer.files[0].type.includes('image')) { return error.textContent = 'You can drop only images.'; }
        if (ev.dataTransfer.files.length != 1) { return error.textContent = 'You can drop only one image.'; }
        const reader = new FileReader();
...
function render(OGimg) {
    ...
    function renderScale() {
        ...
        sizeWidth.oldValue = '';
        sizeHeight.oldValue = '';
        new Array(sizeWidth, sizeHeight).forEach((tbox) => { // input handler
        ...
        let t = 0; let r = 0; let b = 0; let
            l = 0; // top right bottom left
        ...
        function wheelZoom(evt) {
            ...
            const fixed = false;
            if (scale * factor < minzoom) { factor = minzoom / scale; } else if (scale * factor > maxzoom) { factor = maxzoom / scale; }
        ...
        function checkBoundaries() {
            if (scale < minzoom || scale > maxzoom) {
                const factor = scale < minzoom ? minzoom / scale : maxzoom / scale;
                ctxMain.scale(factor, factor);
                scale *= factor;
                // redraw();
            }
            ...
            if (tr.e > side) { ctxMain.translate((side - tr.e) / scale, null); }
            if (tr.f > top) { ctxMain.translate(null, (top - tr.f) / scale); }

            tr = ctxMain.getTransform();
            if (tr.e + img.width * scale < 500 - side) { ctxMain.translate((500 - side - tr.e - img.width * scale) / scale, null); }
            if (tr.f + img.height * scale < 500 - top) { ctxMain.translate(null, (500 - top - tr.f - img.height * scale) / scale); }
        }
        ...
        // const colorPalette = [[131, 251, 45], ..., [54, 61, 24]];
        const ditherMatrix = [
        ...
        function floydSteinbergHSL(sD, closest) {
            for (let p = 0; p < sD.lenght; p += 4) {
                const hsl = RGBToHSL(sD[p], sD[p + 1], sD[p + 2]);
                sD[p] = hsl[0];
                sD[p + 1] = hsl[1];
                sD[p + 2] = hsl[2];
            }
            for (let y = 0; y < canResult.height; y++) {
                for (let x = 0; x < canResult.width; x++) {
                    let i = pxIndex(x, y);
                    const r = HSLToRGB(sD[i], sD[i + 1], sD[i + 2]);
        ...
        function rgbToLab(r, g, b) {
            ...
            let fx; let fy; let fz; let lc; let ac; let bc;
