...
document.onpaste = function (e) { // paste handler
    ...
    if (blob == undefined) { error.textContent = 'Error. Pasted data is not an image.'; return; }
    const reader = new FileReader();
    reader.onload = function (r) {
        const img = new Image();
        error.textContent = 'Loading image...';
        img.onload = function () { design(img); };
...
document.querySelector('#pngInp').onchange = function () { // file input handler
    if (this.files[0] == null) { error.textContent = 'Error. Chose file again.'; return; }

    if (this.files.length > 1) { error.textContent = 'Error. You can select only one file.'; return; }

    const file = this.files[0];
    this.value = null;

    if (file.length === null) { error.textContent = 'Error. Propably file is empty'; return; }
...
drop.ondrop = function (ev) { // drop handlers
    ...
    if (ev.dataTransfer.files) {
        if (ev.dataTransfer.files.length == 0 || !ev.dataTransfer.files[0].type.includes('image')) { error.textContent = 'You can drop only images.'; return; }
        if (ev.dataTransfer.files.length != 1) { error.textContent = 'You can drop only one image.'; return; }
        const reader = new FileReader();
...
function design(OGimg) {
    ...
    const ctxResult = canResult.getContext('2d', { willReadFrequently: true, colorSpace: 'srgb' });
    ctxResult.imageSmoothingEnabled = false;
    const canTemp = document.createElement('canvas');
    const ctxTemp = canTemp.getContext('2d', { willReadFrequently: true, colorSpace: 'srgb' });
    const canSave = document.createElement('canvas');
    const ctxSave = canSave.getContext('2d', { colorSpace: 'srgb' });
    canSave.width = 80;
    canSave.height = 80;
    ...
    function getColors() {
        ...
        const perceivedLightness = (avg.r * 0.2126 + avg.g * 0.7152 + avg.b * 0.0722) / 255;
        document.body.style.setProperty('--bg-accent', perceivedLightness < 0.5 ? 'rgb(255,255,255)' : 'rgb(17,22,28)');
    }

    function renderScale() {
        error.textContent = '';
        for (const item of container.children) { item.classList.remove('active'); }
        setup.classList.add('active');

        canResult.width = Math.min(img.width, 80);
        canResult.height = Math.min(img.height, 80);
        ...
        let minzoom; let
            maxzoom;
        let maxWidth = canResult.width; let maxHeight = canResult.height; let defaultWidth; let
            defaultHeight;
        ...
        sizeWidth.oldValue = '';
        sizeHeight.oldValue = '';
        [sizeWidth, sizeHeight].forEach((tbox) => { // input handler
                ...
                if (/^(?!0)\d{0,2}$/.test(this.value) && !(ratioCheck.checked && !((this == sizeWidth && this.value <= defaultWidth) || (this == sizeHeight && this.value <= defaultHeight))) && ((this == sizeWidth && this.value <= maxWidth) || (this == sizeHeight && this.value <= maxHeight))) {
                    ...
                    if (ratioCheck.checked) {
                        if (this.value != '') { // aspect ratio
                            if (this == sizeWidth) {
                                const h = (img.height * this.value) / img.width;
                                ...
                                const w = (img.width * this.value) / img.height;
                        ...
                        } else if (this == sizeWidth) sizeHeight.value = '';
        ...
        const scaleCheck = document.querySelector('#settings > label:nth-of-type(4) input'); // scale switch
        scaleCheck.onchange = function () { scaleAndDither(); };

        const ditherCheck = document.querySelector('#settings > label:nth-of-type(5) input'); // dither switch
        ditherCheck.onchange = function () { dither(); };

        const alghCheck = document.querySelector('#settings > label:nth-of-type(6) input'); // cie76 or cie94 switch
        alghCheck.onchange = function () { dither(); };

        const hueValue = document.querySelector('#settings > label:nth-of-type(7) :last-child'); // hue rotate
        const hueRange = document.querySelector('#settings > label:nth-of-type(7) input');
        ...
        const saturationValue = document.querySelector('#settings > label:nth-of-type(8) :last-child'); // saturation
        const saturationRange = document.querySelector('#settings > label:nth-of-type(8) input');
        ...
        const brightnessValue = document.querySelector('#settings > label:nth-of-type(9) :last-child'); // brightness
        const brightnessRange = document.querySelector('#settings > label:nth-of-type(9) input');
        ...
        const contrastValue = document.querySelector('#settings > label:nth-of-type(10) :last-child'); // contrast
        const contrastRange = document.querySelector('#settings > label:nth-of-type(10) input');
        ...
        function sizeApply(qzoom = false) {
            if (sizeWidth.value != '') { canResult.width = parseInt(sizeWidth.value); } else canResult.width = parseInt(sizeWidth.getAttribute('placeholder'));
            if (sizeHeight.value != '') { canResult.height = parseInt(sizeHeight.value); } else canResult.height = parseInt(sizeHeight.getAttribute('placeholder'));
            ...
            if (transparency) {
                const c = rgb[document.querySelector('select').selectedIndex];
        ...
        cropButton.onclick = function () {
            ...
            const ar = OGimg.width / OGimg.height;
            if (cropBox2.clientWidth / cropBox2.clientHeight > OGimg.width / OGimg.height) {
                h = cropBox2.clientHeight;
                w = h * ar;
            } else {
                w = cropBox2.clientWidth;
                h = w / ar;
            }
        ...
        cropAccept.onclick = function () {
            ...
            ctxCropped.drawImage(canCrop, lCrop * distorstion, tCrop * distorstion, selector.clientWidth * distorstion, selector.clientHeight * distorstion, 0, 0, canCroped.width, canCroped.height);
        ...
        let tCrop = 0; let rCrop = 0; let bCrop = 0; let lCrop = 0; // top right bottom left
        ...
        let isDown = [false, false, false, false]; const
            offset = [0, 0, 0, 0];
        tEl.addEventListener('mousedown', (e) => {
            isDown[0] = true;
            offset[0] = tCrop - e.clientY;
        });
        rEl.addEventListener('mousedown', (e) => {
            isDown[1] = true;
            offset[1] = rCrop + e.clientX;
        });
        bEl.addEventListener('mousedown', (e) => {
            isDown[2] = true;
            offset[2] = bCrop + e.clientY;
        });
        lEl.addEventListener('mousedown', (e) => {
            isDown[3] = true;
            offset[3] = lCrop - e.clientX;
        });

        cropBox.addEventListener('mousemove', (e) => {
            e.preventDefault();
            if (isDown[0]) {
                if (canCrop.clientHeight - offset[0] - e.clientY - bCrop <= dist) tCrop = canCrop.clientHeight - dist - bCrop;
                else if (e.clientY + offset[0] <= 0) tCrop = 0;
                else tCrop = e.clientY + offset[0];
                selector.style.top = `${tCrop}px`;
            }
            if (isDown[1]) {
                if (canCrop.clientWidth - offset[1] + e.clientX - lCrop <= dist) rCrop = canCrop.clientWidth - dist - lCrop;
                else if (offset[1] - e.clientX <= 0) rCrop = 0;
                else rCrop = offset[1] - e.clientX;
                selector.style.right = `${rCrop}px`;
            }
            if (isDown[2]) {
                if (canCrop.clientHeight - offset[2] + e.clientY - tCrop <= dist) bCrop = canCrop.clientHeight - dist - tCrop;
                else if (offset[2] - e.clientY <= 0) bCrop = 0;
                else bCrop = offset[2] - e.clientY;
                selector.style.bottom = `${bCrop}px`;
            }
            if (isDown[3]) {
                if (canCrop.clientWidth - offset[3] - e.clientX - rCrop <= dist) lCrop = canCrop.clientWidth - dist - rCrop;
                else if (e.clientX + offset[3] <= 0) lCrop = 0;
                else lCrop = offset[3] + e.clientX;
                selector.style.left = `${lCrop}px`;
            }
        });

        cropBox.addEventListener('mouseup', () => { isDown = isDown.fill(false); });
        cropBox.addEventListener('mouseleave', () => { isDown = isDown.fill(false); });
        ...
        redraw();
        function redraw() {
            const p1 = ctxMain.transformedPoint(0, 0);
            const p2 = ctxMain.transformedPoint(canMain.width, canMain.height);
            ctxMain.clearRect(p1.x, p1.y, p2.x - p1.x, p2.y - p1.y);
            ctxMain.drawImage(img, 0, 0);
        }

        let lastX = canMain.width / 2; let lastY = canMain.height / 2; let dragStart; let
            dragged;
        ...
        canMain.addEventListener('mousemove', (evt) => {
            ...
            if (dragStart) {
                shipShape.classList.add('hidden');
                const pt = ctxMain.transformedPoint(lastX, lastY);
        ...
        function wheelZoom(evt) {
            evt.preventDefault();
            shipShape.classList.add('hidden');
            let factor = scaleFactor ** (-evt.deltaY / 40);
            if (zoomScale * factor < minzoom) { factor = minzoom / zoomScale; } else if (zoomScale * factor > maxzoom) { factor = maxzoom / zoomScale; }
        ...
        function checkBoundaries() {
            if (zoomScale < minzoom || zoomScale > maxzoom) {
                const factor = zoomScale < minzoom ? minzoom / zoomScale : maxzoom / zoomScale;
                ctxMain.scale(factor, factor);
                zoomScale *= factor;
                // redraw();
            }
            let tr = ctxMain.getTransform(); const top = 250 - shipShape.height / 2; const
                side = 250 - shipShape.width / 2;
            if (tr.e > side) { ctxMain.translate((side - tr.e) / zoomScale, null); }
            if (tr.f > top) { ctxMain.translate(null, (top - tr.f) / zoomScale); }

            tr = ctxMain.getTransform();
            if (tr.e + img.width * zoomScale < 500 - side) { ctxMain.translate((500 - side - tr.e - img.width * zoomScale) / zoomScale, null); }
            if (tr.f + img.height * zoomScale < 500 - top) { ctxMain.translate(null, (500 - top - tr.f - img.height * zoomScale) / zoomScale); }
        }

        let rangeZoomGlobalID;
        ...
        function rangeZoom() {
            let factor = scaleFactor ** scaleRange.value;
            if (zoomScale * factor < minzoom) { factor = minzoom / zoomScale; } else if (zoomScale * factor > maxzoom) { factor = maxzoom / zoomScale; }
            zoomScale *= factor;
            const pt = ctxMain.transformedPoint(250, 250);
        ...
        function zoomCanvas(newScale) {
            if (newScale >= minzoom) {
                const pt = ctxMain.transformedPoint(250, 250);
                ctxMain.translate(pt.x, pt.y);
                ctxMain.scale(newScale / zoomScale, newScale / zoomScale);
                zoomScale = newScale;
                ctxMain.translate(-pt.x, -pt.y);
                checkBoundaries();
                redraw();
        ...
        async function scaleAndDither() {
            ...
            const scaleImg = scaleCheck.checked ? downgrade : downscale;
            ...
            scaleImg(canTemp, canResult.width, canResult.height, { returnCanvas: true })
                .then((output) => {
        ...
        function dither() {
            ...
            const closest = alghCheck.checked ? closestCIE94 : closestCIE76;
            ...
            if (!ditherCheck.checked) { // no dithering
                ...
                const matrix = [[0, 0, 7 / 16],
                    [3 / 16, 5 / 16, 1 / 16]];
                errorDiffusion(sD, closest, matrix);
                // orderedDither(sD, closest);
            }
            ctxResult.putImageData(scaleD, 0, 0);
        }

        function errorDiffusion(sD, closest, matrix) {
            const start = Math.ceil(matrix[0].length / 2 - 1);
            for (let y = 0; y < canResult.height; y++) {
                for (let x = 0; x < canResult.width; x++) {
                    let i = pxIndex(x, y);
                    const c = closest(sD[i], sD[i + 1], sD[i + 2]);
                    const rEr = sD[i] - rgb[c][0];
                    const gEr = sD[i + 1] - rgb[c][1];
                    const bEr = sD[i + 2] - rgb[c][2];

                    [sD[i], sD[i + 1], sD[i + 2]] = rgb[c];

                    for (let h = 0; h < matrix.length; h++) {
                        const row = matrix[h];
                        for (let w = 0; w < row.length; w++) {
                            if (row[w] == 0) continue;
                            if (y + h < canResult.height && x + w - start >= 0 && x + w - start < canResult.width) {
                                i = pxIndex(x + w - start, y + h);
                                sD[i] += rEr * row[w];
                                sD[i + 1] += gEr * row[w];
                                sD[i + 2] += bEr * row[w];
                            }
        ...
        // const matrix = [[0, 8, 2, 10],
        //     [12, 4, 14, 6],
        //     [3, 11, 1, 9],
        //     [15, 7, 13, 5]];

        function orderedDither(sD, closest) {
            const r = 50;
            for (let y = 0; y < canResult.height; y++) {
                for (let x = 0; x < canResult.width; x++) {
                    const i = pxIndex(x, y);
        ...
        function closestCIE76(r, g, b) {
            let mD = 999999; let
                id;
            const c = rgbToLab(r, g, b);
            for (let i = 0; i < 255; i++) {
                const deltaE = Math.sqrt((c[0] - lab[i][0]) ** 2 + (c[1] - lab[i][1]) ** 2 + (c[2] - lab[i][2]) ** 2);
                if (deltaE < mD) {
                    mD = deltaE;
                    id = i;
                }
        ...
        function closestCIE94(r, g, b) {
            let mD = 999999; let
                id;
            const color1 = rgbToLab(r, g, b);
            for (let i = 0; i < 255; i++) {
                const deltaL = color1[0] - lab[i][0];
                const deltaA = color1[1] - lab[i][1];
                const deltaB = color1[2] - lab[i][2];

                const c1 = Math.sqrt(color1[0] ** 2 + color1[1] ** 2);
                const c2 = Math.sqrt(lab[i][0] ** 2 + lab[i][1] ** 2);
                const deltaC = c1 - c2;
                ...
                const deltaE = o < 0 ? 0 : Math.sqrt(o);
        ...
        function rgbToLab(r, g, b) {
            ...
            const lc = Math.round((116 * fy - 16) * 1000) / 1000;
            const ac = Math.round(500 * (fx - fy) * 1000) / 1000;
            const bc = Math.round(200 * (fy - fz) * 1000) / 1000;
            return [lc, ac, bc];
        }
    }
    ...
    function renderMap() {
        return new Promise((resolve) => {
            ...
            const { data } = ctxResult.getImageData(0, 0, canResult.width, canResult.height);
            for (let i = 0; i < canResult.height; i++) {
                for (let o = 0; o < canResult.width; o++) {
                    const id = (o + i * canResult.width) * 4;
                    const t = getHexColor([data[id], data[id + 1], data[id + 2]]);
                    if (t == undefined) {
                        error.textContent = '(un)There was a problem genering color map. Try again in a diffrent broswer. PS. pls use chrome';
                        return;
                    }
    ...
    function getHexColor(a) {
        for (let i = 0; i < 255; i++) {
            c = rgb[i];
            if (a[0] == c[0] && a[1] == c[1] && a[2] == c[2]) { return i.toString(16).padStart(2, '0').toUpperCase(); }
        }
    }
}
