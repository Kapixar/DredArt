...
topBar.onclick = () => {
    ...
    if (!tool.classList.contains('drag')) mainMenu.classList.toggle('hidden');
...
dialogClose.onclick = () => dialogBox.classList.add('hidden');
...
mainContent.append(mainMenu, dialogBox, insertBox, settings, mainBox);
...
function generateInsertion() {
    ...
    // Paste handler
    document.onpaste = (e) => {
        if (insertBox.classList.contains('hidden')) return;
        e.preventDefault();
...
function validateImage(file = false, url = false) {
    ...
    const img = new Image();
    const imgUrl = url ? (url.startsWith('data') ? url : `https://corsing.kapixar.repl.co?u=${url}`) : URL.createObjectURL(file);
    img.src = imgUrl;
    console.log(imgUrl);
...
async function generateTool(sourceCanvas) {
    ...
    for (let i = 0; i < can.height; i++) {
        for (let o = 0; o < can.width; o++) {
            ...
            const radarPart = (o < xSizesLeg[0] ? 0 : 1) + (i < ySizesLeg[0] ? 0 : 2);
    ...
    const test = [];
    for ([c, j] of Object.entries(imgColors)) {
        if (j.sum === 0) continue;
        test.push(rgb[parseInt(c, 16)]);
    }
    console.log(test);
    ...
    // thumbnail.onload = function () { URL.revokeObjectURL(thUrl); };
    thumbnail.setAttribute('class', 'pixelart');
    stats.style = `background-image: linear-gradient(to right, transparent 0%, #727272 40%), url(${thUrl});`;
    ...
    setupButton.textContent = 'Set up the Holo';
